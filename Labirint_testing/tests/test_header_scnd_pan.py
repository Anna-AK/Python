...
def test_second_panel_fst_point_popup_1(setup_browser):
    """Проверка наличия и кликабильности 1, 2, 6, 8 пунктов подменю *Книги* (не имеют следующего слоя подменю)"""
    page = TestingPage(setup_browser)
    elements = page.are_presence(BOOKS_SCND_A)
    for i in range(len(elements)):
        page.moving_to_elem_click(POINT_BOOKS, elements[i])
        page.wait_page_loaded()
        assert page.assert_pause(page.get_relative_link(), labirint_scnd_panel_books_a[i]), f"Переход на {labirint_scnd_panel_books_a[i]} не прошел"
        elements = page.are_presence(BOOKS_SCND_A)

def test_second_panel_fst_point_popup_3(setup_browser):
    """Проверка наличия и кликабильности подпунктов 3 пункта подменю *Книги* (4 подпункта)"""
    page = TestingPage(setup_browser)
    elements = page.are_presence(BOOKS_SCND_SOST_POINTS_3)
    for i in range(len(elements) - 1):
        i += 1
        page.moving_to_elem(POINT_BOOKS)
        page.moving_to_elem_click(BOOKS_SCND_SOST_3, elements[i])
        page.wait_page_loaded()
        assert page.assert_pause(page.get_relative_link(), labirint_genres + labirint_scnd_panel_books_point3[i - 1]), f"Переход на {labirint_scnd_panel_books_point3[i - 1]} не прошел"
        elements = page.are_presence(BOOKS_SCND_SOST_POINTS_3)

def test_second_panel_fst_point_popup_4(setup_browser):
    """Проверка наличия и кликабильности подпунктов 4 пункта подменю *Книги* (5 подпунктов)"""
    page = TestingPage(setup_browser)
    elements = page.are_presence(BOOKS_SCND_SOST_POINTS_4)
    for i in range(len(elements) - 1):
        i += 1
        page.moving_to_elem(POINT_BOOKS)
        page.moving_to_elem_click(BOOKS_SCND_SOST_4, elements[i])
        page.wait_page_loaded()
        assert page.assert_pause(page.get_relative_link(), labirint_genres + labirint_scnd_panel_books_point4[i - 1]), f"Переход на {labirint_scnd_panel_books_point4[i - 1]} не прошел"
        elements = page.are_presence(BOOKS_SCND_SOST_POINTS_4)

def test_second_panel_fst_point_popup_5(setup_browser):
    """Проверка наличия и кликабильности подпунктов 5 пункта подменю *Книги* (10 подпунктов)"""
    page = TestingPage(setup_browser)
    elements = page.are_presence(BOOKS_SCND_SOST_POINTS_5)
    for i in range(len(elements) - 1):
        i += 1
        print(f'i = {i}')
        page.moving_to_elem(POINT_BOOKS)
        page.moving_to_elem_click(BOOKS_SCND_SOST_5, elements[i])
        page.wait_page_loaded()
        assert page.assert_pause(page.get_relative_link(), labirint_genres + labirint_scnd_panel_books_point5[i - 1]), f"Переход на {labirint_scnd_panel_books_point5[i - 1]} не прошел"
        elements = page.are_presence(BOOKS_SCND_SOST_POINTS_5)

def test_second_panel_fst_point_popup_footer(setup_browser):
    """Проверка наличия и кликабильности ссылок внизу подменю *Книги* (9 ссылок)"""
    page = TestingPage(setup_browser)
    elements = page.are_presence(BOOKS_SCND_FOOT)
    for i in range(len(elements)):
        page.moving_to_elem_click(POINT_BOOKS, elements[i])
        page.wait_page_loaded()
        assert page.assert_pause(page.get_relative_link(), labirint_scnd_panel_books_foot[i]), f"Переход на {labirint_scnd_panel_books_foot[i]} не прошел"
        elements = page.are_presence(BOOKS_SCND_FOOT)
...
def test_second_panel_third_point_all(setup_browser):
    ...
    page.wait_page_loaded()
    assert page.assert_pause(page.get_relative_link(), labirint_school_all), "Переход Школьные предметы не прошел"
...
def test_second_panel_third_point_preschool(setup_browser):
    ...
    page.wait_page_loaded()
    assert page.assert_pause(page.get_relative_link(), labirint_preschool), "Переход на Товары для дошкольныиков не прошел"
